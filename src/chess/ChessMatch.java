...
public class ChessMatch {
	...
	private Piece makeMove(Position source, Position target) {
		...
		if(p instanceof King && target.getColumn() == source.getColumn()+2) {
			makeMove(new Position(source.getRow(), source.getColumn()), new Position(source.getRow(), source.getColumn()+1));
		}
		
		//castling queenside
		if(p instanceof King && target.getColumn() == source.getColumn()-3) {
			makeMove(new Position(source.getRow(), source.getColumn()), new Position(source.getRow(), source.getColumn()-3));
		}
	...
	private void validateSourcePosition(Position source) {
		if(!board.thereIsAPiece(source))
			throw new ChessException("There's no piece in the source position");
		if(currentPlayer!=((ChessPiece)board.piece(source)).getColor())
			throw new ChessException("This piece is not yours");
		if(!board.piece(source).isThereAnyPossibleMove())
			throw new ChessException("There's no possible moves in the source position");
